...
def update_personalColor_to_DB(
    genders, styles, Data_status, save_folderPath, musinsa_type
):
    # 전역변수 선언부
    conn = None
    cur = None
    ...
    for gender in genders:
        for bottomFolder in styles:
            ...
            for file_name in files:
                ...
                print(
                    f"[Thread-{os.getpid()}] Processing {file_name} ({count}/{len(files)})"
                )
                ...
                if analyze_completed_number > musinsa_number:
                    continue
                ...
                personal_rgb = output_dict[
                    "personal_color_rgb"
                ]  # 퍼스널 컬러가 담길 변수
                r, g, b = map(float, personal_rgb)
                h, s, v = rgb_to_hsv(r, g, b)
                musinsa_personal = get_season_tone(
                    personal_rgb
                )  # 의류에 어울리는 퍼스널 컬러

                ## 데이터 업데이트 코드
                file_id = upload_basic(
                    file_name, folderPath + "/" + file_name, gender, bottomFolder
                )
                try :
...
def dataChange(save_folderPath, musinsa_type):
    ...
    Data_status = call_data(musinsa_type)
